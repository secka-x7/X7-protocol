// X7-SV · vaults.js — 10 SVs · 5,000 instances · all chains
// FIX 1: mega_swap emits { chain, swapUSD, log, poolAddr } — log object included
// FIX 2: missed revenue = realistic formula (not × multiplier)
// FIX 3: queueSwap() called so none escape during deploy window
...
// Pool TVLs for realistic missed revenue calculation
const POOL_TVL={
  '0x88e6a0c2ddd26feeb64f039a2c41296fcb3f5640':150e6,
  '0x8ad599c3a0ff1de082011efddc58f1908eb6e6d8':80e6,
  '0xc6962004f452be9203591991d15f6b388e09e8d0':80e6,
  '0x45dda9cb7c25131df268515131f647d726f50608':30e6,
  '0x4c36388be6f416a29c8d8eee81c771ce6be14b5':50e6,
}

const _sv={}
;['sv1','sv2','sv3','sv4','sv5','sv6','sv7','sv8','sv9','sv10'].forEach(k=>(_sv[k]={total:0,count:0,missed:0}))
const _busy={}, _sweep={}
let   _totalMissedReal=0

export const getSVStats=()=>({sv:_sv,total:Object.values(_sv).reduce((s,v)=>s+v.total,0),missed:_totalMissedReal})

// REALISTIC missed revenue: flash_size × net_gap (not swapUSD × 0.0003 × multiplier)
function calcRealisticMissed(swapUSD, poolAddr) {
  const tvl = POOL_TVL[poolAddr?.toLowerCase()] || 50e6
  const impactPct = Math.min(swapUSD/tvl*100, 10)   // cap at 10%
  const gapPct    = impactPct*0.5                    // gap ≈ half the impact
  const flash     = Math.min(tvl*0.08, 20e6)
  const fees      = 0.35                             // combined fees both legs
  const netGap    = Math.max(0, gapPct-fees)
  return Math.min(flash*netGap/100, flash*0.05)      // cap at 5% return
}
...
async function execute(chainName,svKey,calldata,profitEst) {
  ...
  if (!addr) {
    ...
    const missed=calcRealisticMissed(profitEst/0.0003,'')
    _totalMissedReal+=missed
    setConfig('sv_missed_real', _totalMissedReal.toFixed(2))
    emit('missed_rev',{chain:chainName,sv:svKey,amount:profitEst,realistic:missed})
    return null
  }
  ...
  try {
    ...
    if (!txHash){if(_sv[svKey])_sv[svKey].missed+=profitEst;return null}
    if (_sv[svKey]){_sv[svKey].total+=profitEst;_sv[svKey].count++}
    setConfig('sv_total',Object.values(_sv).reduce((s,v)=>s+v.total,0).toFixed(2))
    ...
    try{const r=await import('./revenue.js');r.depositToLPVault(profitEst)}catch{}
    _sweep[chainName]=(_sweep[chainName]||0)+1
    if(_sweep[chainName]>=10||profitEst>1000){_sweep[chainName]=0;sweepProfit(chainName,addr).catch(()=>{})}
    return profitEst
  } catch(e){if(_sv[svKey])_sv[svKey].missed+=profitEst;return null}
...
async function sweepProfit(chainName,addr){
  ...
  await executeBundle(chainName,addr,encodeFunctionData({abi:SWEEP_ABI,functionName:'sweep',args:[tokens,exec]}),0).catch(()=>{})
}
...
async function onMegaSwap(chainName,log,swapUSD){
  ...
  // FIX: pass log + poolAddr in event so bootstrap/scanner can use them
  emit('mega_swap',{chain:chainName,swapUSD,log,poolAddr:log.address})

  // Queue swap so none escape during deploy window
  queueSwap({chain:chainName,swapUSD,log,poolAddr:log.address})

  const baseOpp={tokenIn:chain.usdc,tokenOut:chain.weth,
    amountIn:amounts.abs0>amounts.abs1?amounts.abs0:amounts.abs1,
    buyFee:500,sellFee:3000,profitEst:swapUSD*0.0003}
  const amp=await processPropellers(chainName,baseOpp)
  ...
  for(const opp of cascades){
    const cd=encodeFunctionData({abi:ARB_ABI,functionName:'dexArb',
      args:[chain.usdc,chain.weth,amountIn/2n,opp.fee,opp.fee,0n]})
    await execute(chainName,'sv2',cd,opp.profitUSD)
  }
  await p9MultiChain({swapUSD,buyFee,sellFee},async other=>{
    const oc=getChain(other)
    if(!oc?.weth||!oc?.usdc||other===chainName)return null
    const cd=encodeFunctionData({abi:ARB_ABI,functionName:'dexArb',
      args:[oc.usdc,oc.weth,amountIn/4n,buyFee,sellFee,0n]})
    return execute(other,'sv3',cd,profitEst*0.3)
  })
}
...
function watchChain(chainName){
  ...
  pools.forEach(addr=>ws.subscribe({jsonrpc:'2.0',id:Math.random()*99999|0,
...
async function periodicArb(chainName){
  ...
  const opp=await processPropellers(chainName,{tokenIn:chain.usdc,tokenOut:chain.weth,amountIn:amt,buyFee:500,sellFee:3000,profitEst:100})
  const cd=encodeFunctionData({abi:ARB_ABI,functionName:'dexArb',args:[opp.tokenIn,opp.tokenOut,opp.amountIn,opp.buyFee,opp.sellFee,0n]})
  await execute(chainName,'sv1',cd,opp.profitEst)
}

export function startVaults(){
  console.log('[VAULTS] 10 SVs · 5,000 instances · $100M+ targets')
  try{const s=getConfig('sv_stats');if(s)Object.assign(_sv,JSON.parse(s))}catch{}
  getActiveChains().forEach(c=>watchChain(c.name))
  ;[1,2,3].forEach(tier=>{
    const chains=getTierChains(tier),interval={1:2000,2:5000,3:15000}[tier]
    setInterval(async()=>{
      for(const c of chains){await periodicArb(c.name).catch(()=>{});await new Promise(r=>setTimeout(r,100))}
    },interval)
  })
